...
{
    public class NetCoreClientSender : IClientSender
    {
        readonly Address _address;
        readonly IClientListener _clientListener;
        public NetCoreClientSender(Address address, IClientListener clientListener)
        {
            _address = address;
            _clientListener = clientListener;
        }
        public async Task SendAsync(TransportMessage msg)
        {
            var invokeMessage = msg.GetContent<RemoteInvokeMessage>();
            ...
            var result = await uri.SendJsonAsync(HttpMethod.Get, msg).ReceiveJson<TransportMessage>();
