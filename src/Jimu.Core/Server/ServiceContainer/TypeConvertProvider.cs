...
namespace Jimu.Core.Server.ServiceContainer
{
    ...
    public class TypeConvertProvider : ITypeConvertProvider
    {
        ...
        private static object ComplexTypeConvert(object instance, Type destinationType)
        {
            ...
            if (destinationType != typeof(Guid)) return JsonConvert.DeserializeObject(instance + "", destinationType);
            return Guid.TryParse(instance + "", out var guid) ? guid : JsonConvert.DeserializeObject(instance + "", destinationType);
        }
    }
}